...
def getFeedbackData(feedbackHtml):
    ...
    for element in soup.select('.feedback-list-wrap .feedback-item'):
        ...
        feedbackContent = element.select_one('.buyer-feedback span:first-child').text.strip()
        feedbackTime = element.select_one('.buyer-feedback span:last-child').text.strip()
        feedbackTime = faker.Faker().date_time_this_decade(before_now=True, after_now=False)

        photos = [photo.find('img')['src'] for photo in element.select('.r-photo-list > ul > li')]
